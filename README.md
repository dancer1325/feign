...
* `RedirectionInterceptor`
  * built-in by feign / from redirection responses -- is extracted the -- location header 
...
* `feign-form` extension -- depend on -- `OpenFeign` + 's *concrete* versions
  * | `feign-form` v3.5.0- -> 
    * works with `OpenFeign` v9
  * | `feign-form` v3.5.0 -> 
    * works with `OpenFeign` v10.1.0+ / NO backward compatibility

* if you use
  * [spring-cloud-openfeign](https://github.com/spring-cloud/spring-cloud-openfeign)
    * v2.0.3- -> uses `OpenFeign` v9
    * v2.0.3+ -> uses `OpenFeign` v10
  * `spring-cloud-starter-feign`
    * uses the `OpenFeign` v9
      * Reason: 🧠deprecated 🧠 
...
* use `FormEncoder` + decorate it

    ```java
    SomeApi github = Feign.builder()
                          .encoder(new FormEncoder(new JacksonEncoder()))   // decorate it -- with -- JacksonEncoder 
                          .target(SomeApi.class, "http://api.some.org");
    ```

* specify `Content-Type` header's types 
  * `application/x-www-form-urlencoded`
    ```java
    interface SomeApi {
      ...
      // ALL parameters -- grouped in a -- POJO
  ...
  * `multipart/form-data`
    ...
    interface SomeApi {
      ...
      // EACH param isolated & `File`
      // `File` -- via the File's extension, detect the -- `Content-Type` 
      @RequestLine("POST /send_photo")
      @Headers("Content-Type: multipart/form-data")
      void sendPhoto (@Param("is_public") Boolean isPublic, @Param("photo") File photo);
      
    
      // byte[] parameter           -- will use -- `Content-Type: application/octet-stream` ❓
      @RequestLine("POST /send_photo")
      @Headers("Content-Type: multipart/form-data")
      void sendPhoto (@Param("is_public") Boolean isPublic, @Param("photo") byte[] photo);
    
      // FormData parameter         -- will use the `Content-Type: FormData` &  `FormData`'s `fileName` ❓
      @RequestLine("POST /send_photo")
      @Headers("Content-Type: multipart/form-data")
...
* \+ Spring MultipartFile & Spring Cloud Netflix @FeignClient support
  * TODO:
